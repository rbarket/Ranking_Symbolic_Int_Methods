...
df_test = pd.DataFrame(train_data, columns=['integrand', 'prefix', 'integral', 'label_original'])
df_train_nonelem = pd.DataFrame(train_data_nonelem, columns=['integrand', 'prefix', 'integral', 'label_original'])
df_test_nonelem = pd.DataFrame(test_data_nonelem, columns=['integrand', 'prefix', 'integral', 'label_original'])
...
df_train_nonelem['prefix'] = df_train_nonelem['prefix'].apply(replace_int_with_C)
df_train_nonelem["prefix"] = df_train_nonelem["prefix"].transform(lambda k: tuple(k))  # transforming to tuple is much faster operation
df_train_nonelem.drop_duplicates(subset='prefix', inplace=True)
df_train_nonelem['prefix'] = df_train_nonelem['prefix'].apply(list)

df_test['prefix'] = df_test['prefix'].apply(replace_int_with_C)
df_test_nonelem['prefix'] = df_test_nonelem['prefix'].apply(replace_int_with_C)
...
df_train_nonelem = df_train_nonelem[~ df_train_nonelem['integrand'].apply(contains_unsupported_expressions)]
df_test_nonelem = df_test_nonelem[~ df_test_nonelem['integrand'].apply(contains_unsupported_expressions)]
...
errors_train_nonelem = find_invalid_expressions(df_train_nonelem['prefix'])
errors_test_nonelem = find_invalid_expressions(df_test_nonelem['prefix'])


df_train = df_train[~ df_train['prefix'].isin(errors_train)]
df_test = df_test[~ df_test['prefix'].isin(errors_test)]
df_train_nonelem = df_train_nonelem[~ df_train_nonelem['prefix'].isin(errors_train_nonelem)]
df_test_nonelem = df_test_nonelem[~ df_test_nonelem['prefix'].isin(errors_test_nonelem)]
...
df_train_nonelem['prefix'] = df_train_nonelem['prefix'].apply(lambda x: ['[CLS]'] + x)
df_test_nonelem['prefix'] = df_test_nonelem['prefix'].apply(lambda x: ['[CLS]'] + x)

df_train['label'] = df_train['label_original'].apply(min_max_scale) # scale labels to [0,1] range
df_test['label'] = df_test['label_original'].apply(min_max_scale)
df_train_nonelem['label'] = df_train_nonelem['label_original'].apply(min_max_scale)
df_test_nonelem['label'] = df_test_nonelem['label_original'].apply(min_max_scale)
...
table_train_nonelem = pyarrow.Table.from_pandas(df_train_nonelem)
table_test_nonelem = pyarrow.Table.from_pandas(df_test_nonelem)

pq.write_table(table_train, processed_dir / "train_data.parquet")
pq.write_table(table_test, processed_dir / "test_data.parquet")
pq.write_table(table_train_nonelem, processed_dir / "train_data_nonelem.parquet")
pq.write_table(table_test_nonelem, processed_dir / "test_data_nonelem.parquet")
